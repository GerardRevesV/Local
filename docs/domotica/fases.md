...
| A.13 | ✅ **Fet el 23/09/2026** — 60 h amb els cinc junts en tres tandes (nit, tàper amb sal fins al 73 %, nevera). Dispersió de Td **0,88 → 0,21 °C** per blocs (fora de mostra ~0,3–0,5); correcció d'HR a `custom_templates/calibratge.jinja` (versió `2026-09-23`, desplegada el 23/09 a les 16:48) i T sense corregir. ⏳ **El fred queda obert**: el rang mesurat és 26–30 °C i el soterrani serà de 8–20; es repeteix allà a l'hivern. Números, mètode i dades crues a [calibratge.md](calibratge.md) | 🤝 Tu el fas, jo l'ajusto |
| A.14 | ✅ **Fet** — `scripts/comprova.sh` (368 línies): verifica d'una passada el host, la suspensió, la tapa, Docker, **els dos contenidors un per un**, HA, els sostres de memòria, **les entitats** —les que llegeixen els paquets, els noms fixats amb `default_entity_id` (sense cap `_2`) i totes les del tauler— i Tailscale. És el que es corre **després de cada canvi al host** i abans de donar una porta per tancada | 🤖 Jo |
...
| # | Tasca | Qui |
|---|---|---|
| B.0 | 🔴 **La frontera: `scripts/inicia-serie.sh --de-debo`.** Arxiva la base d'experimentació i comença de zero amb una data d'inici escrita. **Es corre un sol cop**, quan els cinc sensors ja siguin a la seva posició definitiva i **abans** que compti cap dels 28 dies. S'hi nega amb el marcador de calibratge encès (`--apaga-marcador` l'apaga), i si falta o surt duplicat (`…_2`) algun dels sensors de [noms-entitats.md](noms-entitats.md) | 🤝 Jo l'he escrit, tu l'executes |
...
| B.4 | ✅ **Fet** — `packages/rosada.yaml` (637 línies): Td, ΔTd, marge, `history_stats`, `utility_meter` i `sensor.decisio_del_soterrani`. Els blocs dels ventiladors hi són **comentats** fins a la Fase C. *(22/09/2026: la decisió, els paràmetres i els modes passen a `packages/control.yaml` amb la [lògica v2](logica-v2-pla.md), en ombra)* | 🤖 Jo |
...
| C.6 | Comportament d'arrencada de cada endoll: deshumidificador ON, ventiladors OFF. ✅ **La meitat feta el 21/09/2026**: el P110M venia en `off` —no hauria tornat sol després d'un tall— i s'ha posat a **`on`** des d'HA (`select.deshumidificador_power_on_behavior`). ✅ **I l'altra meitat el 26/09/2026**: els dos S110E, a **`off`** (`select.ventilador_N_power_on_behavior`) | 🤝 Jo ho poso, tu ho verifiques |
