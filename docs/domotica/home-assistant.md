...
|---|---|---|
| 1 | Portàtil al router de la SIM, **per cable** si el router té RJ-45 | Que el servidor no depengui del Wi-Fi. ⚠️ *Des del 23/09/2026 va per **Wi-Fi**, per decisió de l'usuari, i Matter està lligat a `wlp0s20f3` → [registre](#registre-dinstallació)* |
...
| 21/09/2026 | 🚀 **Primer desplegament de debò.** El servidor anava **15 commits enrere**. `git pull --ff-only` → `check_config` **net** → `docker compose up -d` (recrea, que és l'únic que aplica `mem_limit`) → `comprova.sh`. **HA va tornar a respondre en 15 s.** Sostres actius i verificats (HA 1.536 MB, `matter-server` 512 MB), `vm.swappiness=10`, **59 entitats**, `sensor.decisio_del_soterrani` viu i decidint, i `comprova.sh` **tot verd**. Entren també el tauler del soterrani i el `lovelace:` que el carrega |
| 21/09/2026 | 🪤 **Parany trobat desplegant:** `docker compose exec -T` s'empassa l'entrada estàndard del guió que el crida. Per una canonada (`ssh host bash -s`, cron) l'`exec` es menja la resta del guió i es penja: **280 s contra 4 s** amb `</dev/null`. Només passa **sense terminal**, que és quan no hi ha ningú mirant. Corregit a `comprova.sh` i escrit al runbook |
...
| 21/09/2026 | 🔗 **Hub H110 emparellat per Matter.** És el **node 1** del `matter-server`, amb subscripció activa i **els sis sensors** penjant-ne: tres T315, dos T310 i el T300 d'inundació. **Els noms del conveni ja hi són aplicats** (`sensor.soterrani_fons_temperatura`, `binary_sensor.soterrani_inundacio`…), que era la casella A.8. Microprogramari llegit del registre: hub **1.3.1**, T315 **1.12.0**, T310 **1.6.0**, T300 **1.9.0** |
| 21/09/2026 | 🔴 **L'endoll P110M no s'empara — tres intents fallits.** HA només diu «Something went wrong»; el `matter-server` diu `PASESession timed out … Expected message type was 33` (nodes 2, 3 i 4): **l'aparell no contesta** la primera petició. **La xarxa queda descartada**: s'anuncia per mDNS amb `CM=2` —finestra oberta— i respon a un ping per IPv4 **i** per IPv6 d'enllaç local en 4 ms. De passada queda confirmat que **és un P110M** (`DT=266`, `DN=Smart Wi-Fi Plug`) amb microprogramari **1.4.3**. Diagnòstic i recepta: [emparellar-matter.md](emparellar-matter.md#-quan-lemparellament-falla-pase-timeout) |
| 21/09/2026 | 🚀 **Desplegat `--primary-interface enp1s0`** al `matter-server` (03:34). `git pull --ff-only` → `check_config` **net** → `docker compose up -d`, que **recrea** el contenidor (és l'únic que aplica un `command` nou) → reinici d'HA per carregar el paquet del calibratge que venia al mateix `main`. Verificat: `docker inspect` mostra els arguments nous, el **hub torna a subscriure's**, HA amb **65 entitats** i `comprova.sh` **tot verd** |
| 21/09/2026 | 🪟 **El navegador no pot emparellar per Matter.** Provat des del Windows: el diàleg *«Afegeix dispositiu Matter»* només diu que cal l'**app Companion** del mòbil, i **no hi ha casella per al codi**. Corregeix el que aquest repositori deia. ⚠️ I no és una drecera: amb l'aparell ja a la Wi-Fi, l'app passa el codi al servidor i **la conversa la fa ell igualment** (`commission_on_network`), o sigui que emparellar des del mòbil **no esquiva** cap problema de xarxa del portàtil |
| 21/09/2026 | ✅ **P110M emparellat: node 9**, a la primera provatura després de desplegar `--primary-interface`. Cinc intents havien mort abans amb `PASE timeout` i el primer de després va entrar net. ⚠️ **No és una prova controlada** —entremig també es va desendollar i tornar a endollar l'endoll—, però la interfície és **l'única cosa que havíem canviat al servidor**. Hi apareixen **12 entitats**: interruptor, **potència (W)**, **energia (kWh)**, tensió, corrent i el comportament d'arrencada |
| 21/09/2026 | 🎉 **El consum per Matter existeix de debò.** `sensor.deshumidificador_potencia` marcant **28,2 W** i `…_energia` **0,481 kWh**, en local i sense compte de Tapo. Era la pregunta oberta que decidia si els **kWh/dia de la Porta B** tenien font: la tenen |
| 21/09/2026 | 🏷️ **Les 12 entitats renombrades al conveni — per l'API de WebSocket, no per la interfície.** `.storage` és de **root** i aquest usuari **no té `sudo` sense contrasenya**, o sigui que editar el registre a mà no era una opció; i tampoc calia. Recepta al [runbook](runbook-servidor.md#-renombrar-entitats-sense-tocar-storage) |
...
| 22/09/2026 | 🚀 **Desplegada la lògica v2, fases 1 a 3: en ombra i amb l'actuació apagada** (12:13–12:16). Els PR #43–#45 anaven apilats i es van fusionar cadascun dins de la seva base, no a `main`; el #47 hi va portar els mateixos commits (diff buit contra el que s'havia fusionat). `check_config` **net** sobre l'arbre exacte de `main` → `git pull --ff-only` (81a5a28 → 8937ec7, 13 commits) → recàrregues, **sense reiniciar**: `reload_custom_templates`, `input_boolean`, `input_number`, `automation` (l'inicialitzador posa els valors de partida: **versió 2**), `input_select`, `timer`, `script` i `template`, totes amb 200. 🪤 El mode es va quedar en `unknown`: *Auto* ja no era una opció i HA no en tria cap altra en recarregar → *Òptim* a mà ([runbook](runbook-servidor.md#-recarregar-un-input_select-que-ha-perdut-lopció-vigent)). Verificat: `sensor.decisio_del_soterrani` en **v2** (diu `calibratge`, com toca) i amb les `entrades`; **139 entitats** (abans 92); `hr_objectiu` i `min_off_deshumidificador` fora; els dos interruptors d'actuació **apagats**; el tauler, amb les cinc vistes, es carrega per l'API; **cap error ni avís** al registre d'HA; `comprova.sh` **tot verd**; i `replica.py --deriva`: 2 files de la v2, **0 diferències**. La prova del dipòsit i el calibratge, **sense tocar**: l'aparell seguia a Manual · 35 %, assecant |
| 23/09/2026 | 📏 **Desplegat el calibratge dels cinc sensors** (versió `2026-09-23`, **16:48:53**). `git pull --ff-only` (e3df54f → 95c2868, 23 commits) → `check_config` **net** → `reload_custom_templates` i **després** `template.reload`, **sense reiniciar**, totes dues amb 200. Verificat: **149 entitats** (abans 139), les deu noves amb el nom exacte i cap `_2`, l'atribut `calibratge` a les deu que fan servir la correcció (els `_cru` sense), els valors idèntics al càlcul, **cap error ni avís** al registre d'HA i `comprova.sh` **tot verd**. El salt, amb els cinc sensors junts a casa: Td de `fons` 16,43 → 17,19 °C; referència interior 17,35 → 17,19 (ara la marca `fons`; abans, `centre`); ΔTd interior − planta baixa 0,09 → 0,31; HR màxima 53,0 → 52,5 %. Queda a la base d'experimentació, que arxiva `inicia-serie.sh`. El marcador del calibratge, encès → [calibratge.md](calibratge.md#registre) |
| 23/09/2026 | 🔧 **Servidor al dia amb `main`** (19:10). `git pull --ff-only` (95c2868 → c9bc505): només guions i docs —el `comprova.sh` de #55 i l'`inicia-serie.sh` de #58—, cap canvi a `config/` ni al compose, **sense recarregar res**. Primera passada del `comprova.sh` nou: **tot correcte**, amb els 27 noms declarats sense cap `_2`, les 84 entitats del tauler i l'avís de sempre (4 referències que esperen maquinari). L'`inicia-serie.sh` nou **no s'ha provat al servidor**: no té mode de prova, i amb `--de-debo` arxiva la base |
| 23/09/2026 | 📶 **El servidor passa a Wi-Fi, i Matter hi segueix** (21:29–21:40). Per decisió de l'usuari, el portàtil deixa el cable i va a la Wi-Fi del router de la SIM (`MERCUSYS_BE8A`): la mateixa xarxa —la porta d'enllaç té la mateixa MAC que pel cable—, amb una IP nova, `192.168.1.106`, i senyal del 98 %. La contrasenya, desada per a tot el sistema (`psk-flags` 0): connecta en arrencar sense cap sessió. **Estalvi d'energia de la Wi-Fi desactivat** al perfil (`802-11-wireless.powersave 2`; el sistema el porta activat per defecte). El cable ja era fora i Matter seguia lligat a `enp1s0`, sense enllaç → #62: `--primary-interface wlp0s20f3` → `git pull --ff-only` (c9bc505 → 04c781c) → `docker compose up -d matter-server`, **sense tocar HA**. El hub (node 1) torna a les 21:37:19 i l'endoll (node 9) a les 21:39:48, amb el deshumidificador a **324 W**. `comprova.sh` **tot correcte** i Tailscale **directe**. L'endoll no responia des de les **08:31** del matí, i els sensors des de les 20:15 → [runbook](runbook-servidor.md#-el-servidor-per-wi-fi-matter-va-lligat-a-una-interfície) |
| 24/09/2026 | 📍 **Marcador del calibratge apagat** (13:51:11): els cinc sensors, repartits per les sales en posicions de prova. La decisió passa de `calibratge` a `repos` —«HR màxima al 56,2 %: dins del 65 % del tram de punta»—, **en ombra**: les dues actuacions segueixen apagades. Els cinc informen des del lloc nou. `inicia-serie.sh` encara no: quan les posicions siguin les definitives → [calibratge.md](calibratge.md#registre) |
| 24–25/09/2026 | 🔌 **Quatre talls de llum** al local per instal·lar els S110E (segons l'usuari): 31, 4, 23 i **81 min**. El portàtil els aguanta **tots amb la bateria**, sense reiniciar-se, i HA no s'atura; cauen el router, el hub i l'endoll, i en tornar el corrent tot es reconnecta sol (la Wi-Fi, el hub per Matter i el deshumidificador). Els S110E, **encara no a HA** → [diari de la sèrie](diari-de-la-serie.md) |
| 26/09/2026 | 🌀 **Els dos S110E, a HA per Matter** (01:18 i 01:21): node **10** (fons → *Ventilador 1*) i node **11** (sala gran → *Ventilador 2*). Emparellats **des de casa i sense mòbil**: el codi temporal de l'app de Tapo, directe al `matter-server` per SSH (`commission_with_code`), **8 s** cadascun. Quin és quin, pel final de la MAC —llegit a l'app— i la IP. Renombrats al conveni al moment, arrencada `off`, microprogramari 1.3.0 al dia. Tancats, fan **7,0 i 4,8 W**. La lògica encara mou els relés de mentida → [emparellar-matter.md](emparellar-matter.md#-des-de-casa-sense-mòbil-el-codi-directe-al-matter-server) |
| 26/09/2026 | 🌬️ **Fase 5 desplegada i provada** (01:40–02:05). `git pull --ff-only` (19e7830 → a4d7b20) → `check_config` **net** → `reload_custom_templates`, **després** `template.reload`, i `history_stats.reload`. `binary_sensor.ventiladors_en_marxa` passa a llegir **watts** dels relés de debò. Experiment amb `actuacio_ventiladors` **encesa** i `delta_td_higiene` 0 → 1 → 0: la decisió canvia en ~15 s, els relés s'obren a les 01:52:51 i es tanquen a les 02:03:47 respectant els temps mínims, i els watts ho segueixen (0,0 W oberts). ⚠️ **L'actuació dels ventiladors es queda encesa** → [logica-v2-pla.md](logica-v2-pla.md#fase-5--els-s110e) |
